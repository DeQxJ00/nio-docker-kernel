...
make -C "$kernel_dir" O="$out_dir" ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- -j"$(nproc)" Image.gz dtbs modules
make -C "$kernel_dir" O="$out_dir" ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- INSTALL_MOD_PATH="$artifact_dir/modules" modules_install
...
make -C "$kernel_dir" O="$out_dir" ARCH=arm64 kernelrelease > "$artifact_dir/kernelrelease"
